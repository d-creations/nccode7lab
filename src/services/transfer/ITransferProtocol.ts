...
export interface ITransferProtocol {
  ...
  listPrograms(ip: string, pathNo: number, port?: number, fileExtensions?: string[]): Promise<TransferProgram[]>;
  uploadProgram(ip: string, pathNo: number, progNum: number, port?: number, fileExtensions?: string[]): Promise<string>;
